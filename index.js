...
let center = {
  x: c.width / 2,
  y: c.height / 2,
  radius: Math.sqrt((c.height * c.height) / 4 + (c.width * c.width) / 4),
};
...
for (let i = 0; i < TOTAL_TRIANGLES * 4; i++) {
  points.push({
...
function calcPerimeterSqr(triangle) {
  return (
    distanceSqr(triangle[0], triangle[1]) +
...
function updatePoints() {
  for (let point of points) {
    ...
    if (focalPoint.x !== 0 || focalPoint.y !== 0) {
      ...
      const forceX = Math.min(
        5,
        Math.max(0, Math.floor(-1 * Math.log2(Math.abs(distX) / c.width) - 2))
      );
      const forceY = Math.min(
        5,
        Math.max(0, Math.floor(-1 * Math.log2(Math.abs(distY) / c.height) - 2))
      );
...
function genGradient(opacity) {
  ...
  const opacityMin = Math.min(255, 0 + opacityOffset)
    .toString(16)
    .padStart(2, "0");
